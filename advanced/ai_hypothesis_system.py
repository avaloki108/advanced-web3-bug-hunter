...
from .hypothesis_engine import HypothesisEngine, VulnerabilityHypothesis, HypothesisGenerationConfig
from .prompt_orchestrator import PromptOrchestrator, PromptStrategy
from .verification_pipeline import VerificationPipeline, VerificationLayer
...
class AIHypothesisSystem:
    ...
    def analyze_contract(self,
                        ...
                        generate_pocs: bool = False) -> AnalysisReport:
        """
        ...
        for i, hypothesis in enumerate(hypotheses, 1):
            print(f"  Verifying hypothesis {i}/{len(hypotheses)}: {hypothesis.description[:60]}...")
        ...
        # Categorize results
        verified = [
            (h, r) for h, r in zip(hypotheses, verification_results)
        ...
        print(f"✓ Verification complete: {len(verified)} verified, {len(rejected)} rejected, {len(uncertain)} uncertain")
        ...
        report = AnalysisReport(
            ...
            creative_hypotheses=len([h for h in hypotheses if h.generated_by.startswith("creative")]),
            refined_hypotheses=len([h for h in hypotheses if h.generated_by.startswith("refinement")]),
            cross_contract_hypotheses=len([h for h in hypotheses if h.generated_by.startswith("cross_contract")]),
    ...
    def _generate_recommendations(self,
        ...
        if verified:
            ...
            if critical:
                recommendations.append(f"• {len(critical)} CRITICAL severity issues - prioritize remediation")
            if high:
                recommendations.append(f"• {len(high)} HIGH severity issues - address soon")
    ...
    def export_report(self, report: AnalysisReport, filepath: str, format: str = "json"):
