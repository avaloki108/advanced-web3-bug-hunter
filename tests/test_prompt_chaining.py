            ...
            return json.dumps([
                {
                    ...
                    "plausibility": "high",
                    "preconditions": ["Flash loan available", "Reentrancy possible"]
                },
                {
            ...
            return json.dumps([
                {
                    ...
                    "attack_sequence": [
                        ...
                        {"step": 2, "action": "Call withdraw", "function": "withdraw"},
                        {"step": 3, "action": "Reenter via fallback", "function": "receive"}
                    ],
...
class TestPromptChainOrchestrator:
    ...
    def test_apply_validations(self):
        ...
        hypotheses = [
            HypothesisItem(id="hyp-001", name="H1", description="D1", plausibility="high", preconditions=[]),
            HypothesisItem(id="hyp-002", name="H2", description="D2", plausibility="low", preconditions=[])
        ]
    ...
    def test_creativity_level_application(self):
        ...
        assert orchestrator.config['stages']['divergent_exploration']['temperature'] == 0.95
        assert orchestrator.config['stages']['technical_validation']['temperature'] == 0.4
...
class TestPromptOptimizer:
    ...
    def test_optimize_based_on_feedback(self):
        ...
        hypotheses = [
            HypothesisItem(id="h1", name="H1", description="D1", plausibility="high", preconditions=[]),
            HypothesisItem(id="h2", name="H2", description="D2", plausibility="medium", preconditions=[]),
            HypothesisItem(id="h3", name="H3", description="D3", plausibility="low", preconditions=[])
        ]
        ...
        assert optimizer.prompt_effectiveness["divergent_exploration"]["total_runs"] == 1
        assert len(optimizer.prompt_effectiveness["divergent_exploration"]["success_rate"]) == 1
    ...
    def test_get_optimization_recommendations(self):
        ...
        # Add some data
        hypotheses = [HypothesisItem(id=f"h{i}", name=f"H{i}", description=f"D{i}", 
                                     plausibility="medium", preconditions=[]) for i in range(10)]
